...
export function NeighborhoodBoardPage({ context = "user" }) {
  const postAsBusiness = context === "business";
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/neighborhood/posts", {
        ...
        body: JSON.stringify({
          ...payload,
          ...(postAsBusiness ? { postAsBusiness: true } : {}),
        }),
      });
            ...
            <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
              <div className="max-w-3xl">
                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-xs font-semibold text-white/90">
                  ...
                  {postAsBusiness ? "İşletme · mahalle" : "Mahalle İletişim Ağı"}
                </div>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-300">
                  {postAsBusiness
                    ? "Paylaşımlar işletmeniz adına yayınlanır (Esnaf rozeti). Gündem, ilan, etkinlik ve yardımlaşmayı buradan yönetin."
                    : "Mahalle paylaşımlarını, ilanları, etkinlikleri ve yardımlaşma kayıtlarını tek panelde görüntüleyin."}
                </p>
              </div>
