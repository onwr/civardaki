...
import { getPlacementLabel, getStatusLabel } from "@/lib/ads/config";
...
export default function AdsTable({ items = [], loading, onEdit, onDelete, pagination = {}, onPageChange }) {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-50">
            {loading ? (
              <tr>
                <td colSpan={7} className="p-12 text-center text-slate-500">Yükleniyor...</td>
              </tr>
            ) : items.length === 0 ? (
              <tr>
                <td colSpan={7} className="p-12 text-center text-slate-500">Reklam bulunamadı.</td>
              </tr>
            ) : (
              items.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50/50">
                  <td className="p-4 font-medium text-slate-900">{row.title || "—"}</td>
                  <td className="p-4 text-sm text-slate-600">{getPlacementLabel(row.placement)}</td>
                  <td className="p-4">
                    <span
