...
export default function QuickAddCustomerModal({ open, onClose, onContinue }) {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Müşteri eklenemedi");

      onContinue?.(data.customer);
    } catch (e) {
      ...
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_24px_60px_rgba(15,23,42,0.22)]">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold">Yeni müşteri oluştur</h2>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50/80 px-4 py-3 text-sm leading-6 text-slate-700">
            Cari hesabı olmayan müşterilerinizi buradan hızlıca ekleyebilirsiniz.
            Diğer detayları daha sonra müşteriler sayfasından güncelleyebilirsiniz.
          </div>
