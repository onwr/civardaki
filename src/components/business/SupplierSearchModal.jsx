...
export default function SupplierSearchModal({ open, onClose, onSelect }) {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/business/suppliers?status=all&q=${encodeURIComponent(q)}`
        );
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          <div>
            ...
            {q.length > 0 && q.length < 3 && (
              <p className="mt-2 text-xs text-amber-700">En az 3 harf yazın.</p>
            )}
          </div>
