...
export async function GET(request) {
  try {
    ...
    if (!userId) return NextResponse.json({ success: false, error: "Kullanıcı bulunamadı." }, { status: 403 });
    ...
    const [items, total] = await Promise.all([
      prisma.support_ticket.findMany({
        where: { creatorType: "USER", userId },
        orderBy: { updatedAt: "desc" },
        skip,
        take: pageSize,
        include: { _count: { select: { messages: true } } },
      }),
      prisma.support_ticket.count({ where: { creatorType: "USER", userId } }),
    ]);
    ...
    console.error("User tickets GET error:", e);
...
export async function POST(request) {
  try {
    ...
    console.error("User tickets POST error:", e);
    return NextResponse.json({ success: false, error: "Sunucu hatası." }, { status: 500 });
  }
}
