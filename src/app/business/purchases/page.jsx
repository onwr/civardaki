...
import SupplierSearchModal from "@/components/business/SupplierSearchModal";
import QuickAddSupplierModal from "@/components/business/QuickAddSupplierModal";

const DOCUMENT_TYPES = [
  { value: "ORDER", label: "Sipariş" },
  { value: "WAYBILLED", label: "İrsaliyeleşmiş" },
  { value: "INVOICED", label: "Faturalaşmış" },
];
...
function ActionButton({ children, onClick, icon: Icon, tone = "dark", className = "" }) {
  const tones = {
...
export default function PurchasesPage() {
  ...
  const fetchPurchases = useCallback(async () => {
    ...
    try {
      ...
      if (selectedDocTypes.length) params.set("documentType", selectedDocTypes.join(","));
      const res = await fetch(`/api/business/purchases?${params}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Liste alınamadı");
      setPurchases(data.purchases || []);
    } catch (e) {
      console.error(e);
      setApiError(e.message || "Alış listesi alınamadı");
      setPurchases([]);
    } finally {
      setLoading(false);
    }
  }, [dateRange, selectedDocTypes, includeCancelled]);
  ...
  const handleSearchSelect = (supplier) => {
    setSearchOpen(false);
    if (supplier?.id) {
      router.push(
        `/business/purchases/yeni?supplierId=${encodeURIComponent(supplier.id)}&supplierName=${encodeURIComponent(supplier.name || "")}`
      );
    }
  };

  const handleQuickAddContinue = (supplier) => {
    ...
    <div className="min-h-[calc(100vh-8rem)] space-y-6 text-[13px] text-slate-700">
        ...
        <div className="relative flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300 md:text-base">
              Kayıtlı veya yeni tedarikçiden alış girişi yapın. Belge tipi ve tarihe göre filtreleyin.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Belge tipi, tarih aralığı ve iptal durumuna göre filtreleyin.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              ...
              {documentTypeOpen && (
                <>
                  <div className="fixed inset-0 z-10" onClick={() => setDocumentTypeOpen(false)} />
                  <div className="absolute left-0 top-full z-20 mt-2 w-72 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_40px_rgba(15,23,42,0.12)]">
                    ...
                    >
                      <input type="checkbox" readOnly checked={selectedDocTypes.length === 0} className="rounded" />
                      Tümünü Seç
                    </button>
                    ...
                    {DATE_RANGES.map((r) => (
                      <button
                        ...
                        className={`block w-full px-4 py-3 text-left text-sm hover:bg-slate-50 ${
                          dateRange === r.value ? "bg-slate-50 font-semibold text-slate-900" : "text-slate-700"
                        }`}
                      >
                        {r.label}
                      </button>
            ...
            <label className="flex cursor-pointer items-center gap-2">
              <input
                ...
                className="rounded"
              />
              <span className="text-sm font-medium text-slate-700">İptalleri de göster</span>
            </label>
            <ActionButton onClick={fetchPurchases} icon={ArrowPathIcon} tone="white" className="rounded-xl px-3 py-2.5">
              Yenile
            </ActionButton>
          </div>
        </div>
      ...
      <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
        <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-4 md:flex-row md:items-center md:justify-between md:px-5">
          <div>
            <h3 className="text-base font-bold text-slate-900">Alış Listesi</h3>
            <p className="mt-1 text-sm text-slate-500">
              Seçili filtrelere göre alış kayıtları
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 font-semibold text-slate-600">
              Belge: {selectedDocTypeText}
            </span>
            <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 font-semibold text-slate-600">
              Tarih: {currentRangeLabel}
            </span>
            <span className="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 font-semibold text-slate-600">
              Kayıt: {purchases.length}
            </span>
          </div>
        </div>
          ...
          <table className="min-w-full border-collapse text-left">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-900 text-white">
                ...
                <th className="px-4 py-3 font-semibold md:px-5">Tedarikçi</th>
                <th className="px-4 py-3 text-right font-semibold md:px-5">Toplam</th>
                <th className="px-4 py-3 text-right font-semibold md:px-5">Ödeme</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-14 text-center md:px-5">
                    <div className="mx-auto max-w-md">
                      ...
                      <h4 className="mt-4 text-base font-bold text-slate-900">Kayıt bulunamadı</h4>
                      <p className="mt-2 text-sm leading-6 text-slate-500">
                        Bu filtrelere uygun alış kaydı bulunmuyor.
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                purchases.map((row, index) => (
                  ...
                  >
                    <td className="px-4 py-3.5 md:px-5">{fmtDate(row.purchaseDate)}</td>
