...
export default function BusinessLayout({ children }) {
  ...
  const isAllowed = canAccessBusinessPanel(user);
  ...
  useEffect(() => {
    if (!isPanelDebugEnabled() || !mounted) return;

    logPanelDebugTable("Business layout — erişim", [
      { alan: "pathname", deger: pathname },
      { alan: "status", deger: status },
      { alan: "isAuthPage", deger: String(isAuthPage) },
      { alan: "user.id", deger: user?.id ?? "(yok)" },
      { alan: "user.role", deger: userRole },
      { alan: "hasBusiness", deger: String(!!user?.hasBusiness) },
      { alan: "businessId", deger: user?.businessId ?? "(yok)" },
      { alan: "isAllowed", deger: String(isAllowed) },
    ]);

    if (status === "authenticated") {
      fetch("/api/debug/panel-access")
        .then((r) => r.json())
        .then((data) => logPanelDebug("Business layout — sunucu teşhisi", data))
        .catch(() => {});
    }
  }, [mounted, status, isAllowed, pathname, isAuthPage, user, userRole]);

  useEffect(() => {
    if (!mounted || isAuthPage) return;

    if (status === "unauthenticated") {
      logPanelDebug("Business layout → yönlendirme", {
        hedef: "/business/login",
        neden: "unauthenticated",
      });
      router.replace(
    ...
    if (status === "authenticated" && !isAllowed) {
      logPanelDebug("Business layout → yönlendirme", {
        hedef: "/user/login",
        neden: "isAllowed=false",
        user,
      });
      router.replace("/user/login");
    }
  }, [mounted, status, isAllowed, pathname, router, isAuthPage, user]);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
              ...
              >
                <div className="min-w-0">
                  <h2 className="text-[15px] font-black tracking-tight leading-none text-white">
                    <span className="lowercase">civar</span>
                    <span className="lowercase text-emerald-300">daki</span>
                  </h2>
          ...
          {collapsed && (
            ...
            >
              <div className="w-full rounded-xl border border-white/10 bg-gradient-to-b from-white/12 to-white/5 px-2 py-2.5 text-center">
                <p className="text-[13px] font-black leading-none tracking-tight text-white">
                  <span className="lowercase">civar</span>
                  <span className="lowercase text-emerald-300">daki</span>
                </p>
                <p className="mt-1.5 text-[7px] font-bold uppercase tracking-[0.22em] text-white/45">
                  işletme
                </p>
              </div>
            </motion.div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex justify-between items-center py-3 sm:py-4 md:py-6">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3 shrink-0">
                  ...
                  <AgendaNotesPanel label="Ajanda & Notlar" />
